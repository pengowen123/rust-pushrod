...
impl WidgetStore {
    ...
    pub fn handle_event(&mut self, widget_id: i32, event: CallbackEvent) -> Option<CallbackEvent> {
        let handles_events = self.widgets[widget_id as usize]
            .widget
            .borrow_mut()
            .handles_events();
    ...
    pub fn inject_event(&mut self, event: CallbackEvent) {
        self.widgets.iter_mut().for_each(|x| {
            let handles_events = x.widget.borrow_mut().handles_events();
    ...
    pub fn draw(&mut self, widget_id: i32, c: Context, g: &mut GlGraphics, original_fbo: GLuint) {
        ...
        for pos in 0..parents_of_widget.len() {
            ...
            if paint_widget.widget.borrow_mut().is_drawable() {
                ...
                // If the object is hidden, do not traverse objects underneath the hidden object, as
                // it stops drawing any children at that point.
                if !is_hidden {
